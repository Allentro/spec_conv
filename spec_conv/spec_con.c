...
#define CHLEN   80      /*character length of filename arrays*/
...
int main(int argc, char *argv[])
{
    ...
    extern int	md;
    extern char ext[NUMOPT][11], exti[NUMOPT][11], fmti[NUMOPT][14], fmt[NUMOPT][14];
...
/*    printf("    Input ext   Output ext  Format\n");
	...
	while (flg == 1)
	{
	    numch = CHMAX;
	    /*if a list file*/
	    if (lst == 1) fn = read_lst(inname,lst);
	    else if (lst == 0)
	    {
	...
	while (flg == 1)
	{
	    ...
	    /*if a list file*/
	    if (lst == 1 && mxsp <= 1) fn = read_lst(inname,lst);
	    else if (lst == 0 && nsp == -1)
	    {
    	    	printf("Type %s filename inc. extension (eg %s):\n",
                        fmti[md-1],exti[md-1]);
...
int ascii_read(char name[])
{
    ...
    if (md == 9 || md == 10)
    {
        ...
        printf("Spectrum date: %s\n",ans);
        
        /*get and print real/live time*/
        skip_lines(fsp, 1);
        rlt[0] = 0.0; rlt[1] = 0.0;
        get_pars_file(fsp, rlt, 2);
        printf("    Real time:%.2f s, live time %.2f s\n",rlt[0],rlt[1]);
        
        /*get channel number*/
        skip_lines(fsp, 1);
        rlt[0] = 0.0; rlt[1] = 0.0;
        get_pars_file(fsp, rlt, 2);
        printf("    First chan:%.2f, last chan %.2f\n",rlt[0],rlt[1]);
    }
    
    /*Determine if spectrum is 1 or 2 col. ascii format & set 1 col flag*/
    if ( (ascii = col_determ(fsp)) == 0) ascii = 1;
...
void ascii_write(char name[], int numch)
{
    ...
    /*write .txt file*/
    for (j = 0; j < numch; j++) fprintf(fasc, "%d %d\n", j, (int)spectrum[j]);
...
int col_determ(FILE *file)
{
    ...
    while (1)
    {
    	...
    	while ( (hash = fgetc(file)) != '\n' )
    	{
    		...
    		if ( (blnk != 0) && (col != 2) )
    		    col = 2;
    		/*otherwise if number is third column set 1 col. flag*/
    		/*and hope for best*/
    		else
    		{
    		    col = 1;
...
/*===========================================================================*/
/* maestro_read: read the maestero format spectrum */
/*****************************************************************************/
int maestro_read(char name[])
{
    int *counts, i = 0;
